...
.controller('DashCtrl', function($scope, $rootScope, IonicLogin, $state, $http, $ionicLoading) {
 ...
 // GET VEHICLES FROM DB
  ...
  this.markExpiring = function (){
    console.log('mark expiring');
    console.log(self.data.expiringVehicleList.length);
  
    var reminder = new Date();
		reminder = reminder.setMonth(reminder.getMonth() + 1) ; //set reminder to one month ago
    
    for (var i=0; i < self.data.expiringVehicleList.length; i++){
        console.log(self.data.expiringVehicleList[i]);
      	self.data.expiringVehicleList[i].registrationRed = (new Date(self.data.expiringVehicleList[i].registrationExpiresOn) < reminder) ? 'red':'' ;
        self.data.expiringVehicleList[i].insuranceRed = (new Date(self.data.expiringVehicleList[i].insuranceExpiresOn) < reminder) ? 'red':'';
        self.data.expiringVehicleList[i].secondInsuranceRed = (new Date(self.data.expiringVehicleList[i].secondInsuranceExpiresOn) < reminder)  ? 'red':'';
        self.data.expiringVehicleList[i].cargoInsuranceRed = (new Date(self.data.expiringVehicleList[i].cargoInsuranceExpiresOn) < reminder) ? 'red':'';
        self.data.expiringVehicleList[i].techInspectionRed = (new Date(self.data.expiringVehicleList[i].techInspectionExpiresOn) < reminder) ? 'red':'';
        self.data.expiringVehicleList[i].maintenanceInspectionRed = (new Date(self.data.expiringVehicleList[i].maintenanceInspectionExpiresOn) < reminder) ? 'red':'';
    }
    console.log(self.data.expiringVehicleList);
}
...
.controller('IonicLogin', function($scope, IonicLogin, $ionicLoading, $cordovaOauth, $http, $location, $window) {
 ...
 var captchaVerified = true ;
  ...
  this.login = function(){
   ...
   this.signUp = function(){
     if (captchaVerified)
        IonicLogin.signUp(self.data.email_signup, self.data.password_signup);
  }
